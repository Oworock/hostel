...
>
    ...
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div class="flex justify-between items-center h-16">
            ...
            <div class="hidden md:flex items-center space-x-8">
                <a href="{{ route('public.rooms.index') }}" class="text-gray-700 dark:text-slate-200 hover:text-blue-600 dark:hover:text-blue-400">{{ __('Book Rooms') }}</a>
                @if(auth()->check())
                    @if(auth()->user()->isStudent())
                        <a href="{{ route('student.bookings.available') }}" class="text-gray-700 dark:text-slate-200 hover:text-blue-600 dark:hover:text-blue-400">{{ __('Browse Rooms') }}</a>
                        <a href="{{ route('student.bookings.index') }}" class="text-gray-700 dark:text-slate-200 hover:text-blue-600 dark:hover:text-blue-400">{{ __('My Bookings') }}</a>
                    @elseif(auth()->user()->isManager())
                        <a href="{{ route('manager.rooms.index') }}" class="text-gray-700 dark:text-slate-200 hover:text-blue-600 dark:hover:text-blue-400">{{ __('Rooms') }}</a>
                        <a href="{{ route('manager.bookings.index') }}" class="text-gray-700 dark:text-slate-200 hover:text-blue-600 dark:hover:text-blue-400">{{ __('Bookings') }}</a>
                    @elseif(auth()->user()->isAdmin())
                        <a href="{{ route('filament.admin.resources.hostels.index') }}" class="text-gray-700 dark:text-slate-200 hover:text-blue-600 dark:hover:text-blue-400">{{ __('Hostels') }}</a>
                    @endif

                    <a href="{{ route('dashboard') }}" class="text-gray-700 dark:text-slate-200 hover:text-blue-600 dark:hover:text-blue-400">{{ __('Dashboard') }}</a>
                ...
                @else
                    <a href="{{ route('login') }}" class="text-gray-700 dark:text-slate-200 hover:text-blue-600 dark:hover:text-blue-400">{{ __('Login') }}</a>
                    <a href="{{ route('register') }}" class="bg-blue-600 text-white px-4 py-2 rounded-lg hover:bg-blue-700">{{ __('Register') }}</a>
                    <button type="button" @click="toggleTheme()" class="inline-flex items-center gap-2 rounded-full border border-gray-300 dark:border-slate-700 px-2 py-1.5 text-xs font-semibold text-gray-700 dark:text-slate-200 hover:bg-gray-50 dark:hover:bg-slate-800" title="{{ __('Toggle dark mode') }}">
    ...
    <div id="mobile-menu" x-show="open" x-cloak x-transition class="md:hidden border-t border-gray-200 dark:border-slate-800 bg-white dark:bg-slate-900">
        <div class="px-4 py-3 space-y-2">
            @if(auth()->check())
                <a href="{{ route('public.rooms.index') }}" class="block text-gray-700 dark:text-slate-200 hover:text-blue-600 dark:hover:text-blue-400">{{ __('Book Rooms') }}</a>
                @if(auth()->user()->isStudent())
                    <a href="{{ route('student.bookings.available') }}" class="block text-gray-700 dark:text-slate-200 hover:text-blue-600 dark:hover:text-blue-400">{{ __('Browse Rooms') }}</a>
                    <a href="{{ route('student.bookings.index') }}" class="block text-gray-700 dark:text-slate-200 hover:text-blue-600 dark:hover:text-blue-400">{{ __('My Bookings') }}</a>
                @elseif(auth()->user()->isManager())
                    <a href="{{ route('manager.rooms.index') }}" class="block text-gray-700 dark:text-slate-200 hover:text-blue-600 dark:hover:text-blue-400">{{ __('Rooms') }}</a>
                    <a href="{{ route('manager.bookings.index') }}" class="block text-gray-700 dark:text-slate-200 hover:text-blue-600 dark:hover:text-blue-400">{{ __('Bookings') }}</a>
                @elseif(auth()->user()->isAdmin())
                    <a href="{{ route('filament.admin.resources.hostels.index') }}" class="block text-gray-700 dark:text-slate-200 hover:text-blue-600 dark:hover:text-blue-400">{{ __('Hostels') }}</a>
                @endif

                <a href="{{ route('dashboard') }}" class="block text-gray-700 dark:text-slate-200 hover:text-blue-600 dark:hover:text-blue-400">{{ __('Dashboard') }}</a>
                <a href="{{ auth()->user()->isStudent() ? route('student.profile.edit') : (auth()->user()->isManager() ? route('manager.profile.edit') : (\Illuminate\Support\Facades\Route::has('filament.admin.pages.profile') ? route('filament.admin.pages.profile') : route('dashboard'))) }}" class="block text-gray-700 dark:text-slate-200 hover:text-blue-600 dark:hover:text-blue-400">{{ __('Edit Profile') }}</a>
                <form method="POST" action="{{ route('logout') }}">
                    @csrf
                    <button type="submit" class="w-full text-left text-red-700 dark:text-red-300 hover:text-red-800">{{ __('Sign Out') }}</button>
                </form>
            @else
                <a href="{{ route('public.rooms.index') }}" class="block text-gray-700 dark:text-slate-200 hover:text-blue-600 dark:hover:text-blue-400">{{ __('Book Rooms') }}</a>
                <a href="{{ route('login') }}" class="block text-gray-700 dark:text-slate-200 hover:text-blue-600 dark:hover:text-blue-400">{{ __('Login') }}</a>
                <a href="{{ route('register') }}" class="block text-gray-700 dark:text-slate-200 hover:text-blue-600 dark:hover:text-blue-400">{{ __('Register') }}</a>
                <button type="button" @click="toggleTheme()" class="block w-full text-left text-gray-700 dark:text-slate-200 hover:text-blue-600 dark:hover:text-blue-400">{{ __('Theme') }}:
                    <span x-show="isDark()">{{ __('Dark') }}</span>
                    <span x-show="!isDark()">{{ __('Light') }}</span>
                </button>
            @endif
        </div>
    </div>
