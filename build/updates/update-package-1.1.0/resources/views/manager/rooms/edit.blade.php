                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    <div>
                        ...
                        <select id="type" name="type" class="w-full px-4 py-2.5 border border-slate-300 dark:border-slate-700 bg-white dark:bg-slate-800 text-slate-900 dark:text-slate-100 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500 @error('type') border-red-500 @enderror" required>
                            <option value="single" @selected(old('type', $room->type) === 'single')>Single Occupancy (1)</option>
                            <option value="double" @selected(old('type', $room->type) === 'double')>Double Occupancy (2)</option>
                            <option value="triple" @selected(old('type', $room->type) === 'triple')>Triple Occupancy (3)</option>
                            <option value="quad" @selected(old('type', $room->type) === 'quad')>Quad Occupancy (4)</option>
                            <option value="quint" @selected(old('type', $room->type) === 'quint')>Quintuple Occupancy (5)</option>
                            <option value="sext" @selected(old('type', $room->type) === 'sext')>Sextuple Occupancy (6)</option>
                            <option value="sept" @selected(old('type', $room->type) === 'sept')>Septuple Occupancy (7)</option>
                            <option value="oct" @selected(old('type', $room->type) === 'oct')>Octuple Occupancy (8)</option>
                        </select>
