...
<body class="h-full bg-gray-50 dark:bg-slate-950 text-gray-900 dark:text-slate-100 antialiased">
    @php
        ...
        if ($currentUser && ($currentUser->isStudent() || $currentUser->isManager())) {
            ...
            // If the pivot table exists, show unseen popups or popups updated since last seen.
            if (\Illuminate\Support\Facades\Schema::hasTable('popup_announcement_user')) {
                $query->where(function ($q) use ($currentUser): void {
                    $q->whereDoesntHave('seenByUsers', fn ($seen) => $seen->where('users.id', $currentUser->id))
                        ->orWhereHas('seenByUsers', fn ($seen) => $seen
                            ->where('users.id', $currentUser->id)
                            ->whereColumn('popup_announcement_user.seen_at', '<', 'popup_announcements.updated_at'));
                });
            }

            $loginPopup = $query->latest('id')->first();
        }
