<x-dashboard-layout :title="__('Room Change Requests')">
    ...
    <div class="uniform-page">
        <div class="uniform-header">
            <h1 class="text-3xl font-bold text-slate-900 dark:text-slate-100">{{ __("Room Change Requests") }}</h1>
            <p class="text-slate-600 dark:text-slate-300 mt-1">{{ __('Review student requests to move to another room in your managed hostels.') }}</p>
        </div>
        ...
        <div class="uniform-grid-2">
            @forelse($requests as $request)
                <article class="uniform-card p-5">
                    ...
                    <div class="mt-4 space-y-2 text-sm">
                        <p class="text-slate-700 dark:text-slate-200"><span class="font-medium">Current:</span> {{ $request->currentRoom->hostel->name ?? '-' }} - {{ $request->currentRoom->room_number ?? '-' }}</p>
                        <p class="text-slate-700 dark:text-slate-200"><span class="font-medium">Requested:</span> {{ $request->requestedRoom->hostel->name ?? '-' }} - {{ $request->requestedRoom->room_number ?? '-' }}</p>
                    ...
                    @if($request->status === 'pending_manager_approval')
                        <div class="mt-4 space-y-3">
                            <form action="{{ route('manager.room-change.approve', $request) }}" method="POST" class="space-y-2">
                                @csrf
                                @method('PATCH')
                            ...
                            <form action="{{ route('manager.room-change.reject', $request) }}" method="POST" class="space-y-2">
                                @csrf
                                @method('PATCH')
            ...
            @empty
                <div class="col-span-full uniform-card p-8 text-center text-slate-600 dark:text-slate-300">{{ __("No room change requests found.") }}</div>
            @endforelse
        </div>
