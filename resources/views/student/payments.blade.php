<x-dashboard-layout :title="__('My Payments')">
    ...
    <div class="max-w-6xl mx-auto space-y-6">
        <div>
            ...
            <p class="text-slate-600 dark:text-slate-300 mt-1">{{ __('Review your payment records and outstanding balance.') }}</p>
        </div>

        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div class="bg-white dark:bg-slate-900 rounded-lg shadow-md border border-transparent dark:border-slate-800 p-6">
                <p class="text-slate-600 dark:text-slate-300 text-sm">{{ __("Total Paid") }}</p>
                <p class="text-3xl font-bold text-green-600 dark:text-green-400">{{ formatCurrency($totalPaid) }}</p>
            </div>
            <div class="bg-white dark:bg-slate-900 rounded-lg shadow-md border border-transparent dark:border-slate-800 p-6">
                <p class="text-slate-600 dark:text-slate-300 text-sm">{{ __("Outstanding Balance") }}</p>
        ...
        @if($pendingPayments->count() > 0)
            <section class="bg-white dark:bg-slate-900 rounded-lg shadow-md border border-transparent dark:border-slate-800 p-6">
                <h2 class="text-xl font-semibold text-slate-900 dark:text-slate-100 mb-4">{{ __("Pending Payments") }}</h2>
                <div class="space-y-3">
                    @foreach($pendingPayments as $payment)
                        <div class="rounded-lg border border-slate-200 dark:border-slate-700 bg-slate-50 dark:bg-slate-800 p-4 flex items-center justify-between gap-3">
                            ...
                            <form action="{{ route('student.payments.complete', $payment->id) }}" method="POST">
                                @csrf
                                <button type="submit" class="px-4 py-2 bg-blue-600 text-white rounded-lg hover:bg-blue-700 text-sm font-medium">{{ __("Complete Payment") }}</button>
                            </form>
                        </div>
                    @endforeach
                </div>
        ...
        <section class="bg-white dark:bg-slate-900 rounded-lg shadow-md border border-transparent dark:border-slate-800 overflow-hidden">
            <div class="px-6 py-4 border-b border-slate-200 dark:border-slate-700 bg-slate-50 dark:bg-slate-800">
                <h2 class="text-lg font-semibold text-slate-900 dark:text-slate-100">{{ __("Payment History") }}</h2>
            </div>
                ...
                <table class="w-full min-w-[860px]">
                    <thead class="bg-slate-50 dark:bg-slate-800 border-b border-slate-200 dark:border-slate-700">
                        <tr>
                            ...
                            <th class="px-6 py-3 text-left text-xs font-semibold uppercase tracking-wide text-slate-600 dark:text-slate-300">{{ __("Booking") }}</th>
                            ...
                            <th class="px-6 py-3 text-left text-xs font-semibold uppercase tracking-wide text-slate-600 dark:text-slate-300">{{ __("Receipt") }}</th>
                        </tr>
                    </thead>
                    <tbody class="divide-y divide-slate-200 dark:divide-slate-800">
                        @forelse($paymentHistory as $payment)
                            <tr class="hover:bg-slate-50 dark:hover:bg-slate-800/60">
                                ...
                                <td class="px-6 py-4 text-sm">
                                    @if(in_array($payment->status, ['completed', 'paid']))
                                        <a href="{{ route('student.payments.receipt', $payment->id) }}" class="text-blue-600 dark:text-blue-400 hover:underline font-medium">{{ __("Download Receipt") }}</a>
                                    @else
                        ...
                        @empty
                            <tr>
                                <td colspan="6" class="px-6 py-8 text-center text-slate-600 dark:text-slate-300">{{ __("No payment history.") }}</td>
                            </tr>
                        @endforelse
                    </tbody>
                </table>
        ...
        <section class="bg-white dark:bg-slate-900 rounded-lg shadow-md border border-transparent dark:border-slate-800 p-6">
            <h3 class="text-lg font-semibold text-slate-900 dark:text-slate-100 mb-2">{{ __("Need Help?") }}</h3>
            <p class="text-slate-600 dark:text-slate-300">{{ __('If you have questions about your payments, contact support.') }}</p>
        </section>
    </div>
</x-dashboard-layout>
