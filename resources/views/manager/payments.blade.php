<x-dashboard-layout :title="__('Payments')">
    ...
    <div class="max-w-6xl mx-auto space-y-6">
        <div>
            <h1 class="text-3xl font-bold text-slate-900 dark:text-slate-100">{{ __("Payment Management") }}</h1>
            <p class="text-slate-600 dark:text-slate-300 mt-1">{{ __('Track settled, partial, and outstanding student payments.') }}</p>
        </div>

        <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
            <div class="bg-white dark:bg-slate-900 rounded-lg shadow-md border border-transparent dark:border-slate-800 p-6">
                <p class="text-slate-600 dark:text-slate-300 text-sm">{{ __("Paid Full") }}</p>
                <p class="text-3xl font-bold text-green-600 dark:text-green-400">{{ $paidFull }}</p>
            </div>
            <div class="bg-white dark:bg-slate-900 rounded-lg shadow-md border border-transparent dark:border-slate-800 p-6">
                <p class="text-slate-600 dark:text-slate-300 text-sm">{{ __("Partial Payment") }}</p>
                <p class="text-3xl font-bold text-amber-600 dark:text-amber-400">{{ $partialPayment }}</p>
            </div>
            <div class="bg-white dark:bg-slate-900 rounded-lg shadow-md border border-transparent dark:border-slate-800 p-6">
                <p class="text-slate-600 dark:text-slate-300 text-sm">{{ __("Still Owing") }}</p>
        ...
        <section class="bg-white dark:bg-slate-900 rounded-lg shadow-md border border-transparent dark:border-slate-800 p-6">
            <form method="GET" action="{{ route('manager.payments.index') }}" class="flex flex-wrap gap-3 items-end">
                <div>
                    <label class="block text-xs font-semibold uppercase tracking-wide text-slate-500 dark:text-slate-400 mb-2">{{ __("Status") }}</label>
                    <select name="status" class="px-4 py-2.5 border border-slate-300 dark:border-slate-700 rounded-lg bg-white dark:bg-slate-800 text-slate-900 dark:text-slate-100 focus:ring-2 focus:ring-blue-500">
                        ...
                        <option value="paid_full" {{ request('status') === 'paid_full' ? 'selected' : '' }}>{{ __("Paid Full") }}</option>
                        <option value="partial" {{ request('status') === 'partial' ? 'selected' : '' }}>{{ __("Partial Payment") }}</option>
                        <option value="owing" {{ request('status') === 'owing' ? 'selected' : '' }}>{{ __("Still Owing") }}</option>
                    </select>
                </div>
                ...
                <table class="w-full min-w-[860px]">
                    <thead class="bg-slate-50 dark:bg-slate-800 border-b border-slate-200 dark:border-slate-700">
                        <tr>
                            ...
                            <th class="px-6 py-3 text-left text-xs font-semibold uppercase tracking-wide text-slate-600 dark:text-slate-300">{{ __("Total Amount") }}</th>
                    ...
                    <tbody class="divide-y divide-slate-200 dark:divide-slate-800">
                            ...
                            <tr class="hover:bg-slate-50 dark:hover:bg-slate-800/60">
                                ...
                                <td class="px-6 py-4 text-sm">
                                    @if($payment->payment_status === 'paid_full')
                                        <span class="px-3 py-1 bg-green-100 text-green-800 dark:bg-green-900/40 dark:text-green-300 rounded-full text-xs font-semibold">{{ __("Paid Full") }}</span>
                        ...
                        @empty
                            <tr>
                                <td colspan="6" class="px-6 py-8 text-center text-slate-600 dark:text-slate-300">{{ __('No payments found.') }}</td>
                            </tr>
                        @endforelse
                    </tbody>
                </table>
