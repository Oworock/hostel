                        ...
                        <select id="requested_room_id" name="requested_room_id" class="w-full px-4 py-2.5 border border-slate-300 dark:border-slate-700 rounded-lg bg-white dark:bg-slate-800 text-slate-900 dark:text-slate-100" required>
                            ...
                            @foreach($availableRooms as $room)
                                <option value="{{ $room->id }}">
                                    {{ $room->room_number }} - {{ ucfirst($room->type) }} ({{ $room->availableBeds()->count() }} beds available)
                                </option>
                            @endforeach
                        </select>
                    </div>
                        ...
                        @forelse($requests as $request)
                            <tr class="hover:bg-slate-50 dark:hover:bg-slate-800/60">
                                <td class="px-6 py-4 text-sm text-slate-900 dark:text-slate-100">
                                    {{ $request->currentRoom->hostel->name ?? '-' }} - {{ $request->currentRoom->room_number ?? '-' }}
                                </td>
                                <td class="px-6 py-4 text-sm text-slate-900 dark:text-slate-100">
                                    {{ $request->requestedRoom->hostel->name ?? '-' }} - {{ $request->requestedRoom->room_number ?? '-' }}
                                </td>
