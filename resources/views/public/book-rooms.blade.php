...
<section class="py-8 md:py-12">
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 space-y-6">
        <div class="flex flex-col md:flex-row md:items-center md:justify-between gap-2">
            <div>
                <h1 class="text-5xl font-bold text-slate-900 dark:text-slate-100">{{ __("Available Rooms") }}</h1>
                <p class="text-slate-600 dark:text-slate-300 mt-1">{{ __(':count rooms available for booking.', ['count' => $rooms->total()]) }}</p>
            </div>
            @guest
                <div class="flex items-center gap-2">
                    <a href="{{ route('register') }}" class="px-4 py-2 rounded-lg border border-blue-600 text-blue-700 dark:text-blue-300 font-semibold">{{ __('Sign up') }}</a>
                    <a href="{{ route('login') }}" class="px-4 py-2 rounded-lg bg-blue-600 text-white font-semibold">{{ __('Log in') }}</a>
                </div>
            @endguest
        </div>
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-6">
            @forelse($rooms as $room)
                <x-room-listing-card variant="booking" :room="$room" :action-url="route('public.rooms.book', $room)" :action-label="__('Book Now')" />
            @empty
                <div class="col-span-full rounded-xl border border-slate-200 dark:border-slate-700 bg-slate-50 dark:bg-slate-800 p-10 text-center text-slate-600 dark:text-slate-300">{{ __("No rooms match your filters.") }}</div>
            @endforelse
        </div>
