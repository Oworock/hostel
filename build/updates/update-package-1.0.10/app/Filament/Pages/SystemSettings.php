...
class SystemSettings extends Page
{
    ...
    public function save(): void
    {
        ...
        $data['registration_school_options_json'] = json_encode(
            collect(preg_split('/\r\n|\r|\n/', (string) ($data['registration_school_options_text'] ?? '')) ?: [])
                ->map(fn ($school) => trim((string) $school))
                ->filter()
                ->unique()
                ->values()
        ...
        foreach ($data as $key => $value) {
            if (!in_array($key, ['test_phone', 'sms_payload_template', 'sms_custom_headers', 'webhook_events', 'registration_fields', 'registration_required_fields', 'registration_custom_fields', 'notification_templates', 'trimester_eligible_schools_text', 'registration_school_options_text'], true)) {
