...
class SystemUpdatePage extends Page
{
                ...
                Forms\Components\Section::make('Update Decision')
                    ...
                    ->schema([
                        ...
                        Forms\Components\Toggle::make('acknowledge_custom_warning')
                            ...
                            ->default(false),
                        Forms\Components\Placeholder::make('update_action_note')
                            ->content('Use the Update Now or Decline buttons in the preview section below.'),
                    ]),
            ])
            ->statePath('data');
    }
    ...
    protected function getHeaderActions(): array
    {
        return [
            ...
            Action::make('previewUploadedUpdate')
                ->label('Preview Uploaded File')
                ->icon('heroicon-o-eye')
                ->visible(fn (): bool => $this->normalizeStoredFileValue($this->data['update_zip_path'] ?? null) !== '')
                ->action(fn (): null => ($this->previewUploadedUpdate() ?? null)),
            Action::make('continueUpdate')
                ->label('Continue Update')
                ->icon('heroicon-o-check-circle')
                ->color('success')
                ->visible(fn (): bool => !empty($this->previewReport))
                ->action(fn (): null => ($this->continueUpdate() ?? null)),
            Action::make('declineUpdate')
                ->label('Decline Update')
                ->icon('heroicon-o-x-circle')
                ->color('gray')
                ->visible(fn (): bool => !empty($this->previewReport))
                ->action(fn (): null => ($this->declineUpdate() ?? null)),
        ];
    }
    ...
    private function processDecision(string $decision): void
    {
        ...
        if (!in_array($decision, ['continue', 'decline'], true)) {
            Notification::make()->warning()->title('Invalid update decision')->send();
        ...
        try {
            ...
            if (is_string($version) && $version !== '') {
                SystemSetting::setSetting('system_app_version', $version);
            }

            $this->runPostUpdateMaintenance();

            $this->previewReport = null;
            $this->data['current_version'] = (string) SystemSetting::getSetting('system_app_version', '1.0.0');
            $this->form->fill($this->data);
    ...
    {
        $this->processDecision('continue');
    }

    public function declineUpdate(): void
    {
        $this->processDecision('decline');
    }
    ...
    private function runPostUpdateMaintenance(): void
    {
        $token = $this->resolveUpdatePingToken();
        $pingUrl = route('system.update.ping', ['token' => $token]);
    ...
    private function resolveUpdatePingToken(): string
    {
