<x-dashboard-layout :title="__('Asset Management')">
    <x-slot name="sidebar">
    ...
    <div class="uniform-page space-y-6">
        <div class="uniform-header">
            <h1 class="text-3xl font-bold text-slate-900 dark:text-slate-100">{{ __("Hostel Assets") }}</h1>
            <p class="text-slate-600 dark:text-slate-300">{{ __('Add assets, report issues, and request inter-hostel movement with approvals.') }}</p>
        </div>

        <section class="uniform-card p-6">
            <div class="flex flex-wrap items-center justify-between gap-3">
                <p class="text-slate-600 dark:text-slate-300">{{ __('Need to register a new asset? Use the dedicated add page.') }}</p>
            ...
            <p class="text-xs text-amber-700 dark:text-amber-300 mt-3">{{ __('Managers can add assets but cannot edit any asset record after creation.') }}</p>
        </section>

        <section class="uniform-card p-6">
            <h2 class="text-xl font-semibold text-slate-900 dark:text-slate-100 mb-4">{{ __("Incoming Movement Requests") }}</h2>
            <div class="space-y-3">
                @forelse($incomingMovements as $movement)
                    <div class="rounded-lg border border-slate-200 dark:border-slate-700 p-3">
                        ...
                        <form method="POST" action="{{ route('manager.assets.movements.respond', $movement) }}" class="mt-3 grid grid-cols-1 md:grid-cols-3 gap-3">
                            @csrf
                            <textarea name="receiving_manager_note" rows="2" placeholder="{{ __("Comment") }}" class="md:col-span-3 px-3 py-2 rounded-lg border border-slate-300 dark:border-slate-700 bg-white dark:bg-slate-900"></textarea>
                            <button name="decision" value="accept" class="bg-green-600 text-white px-4 py-2 rounded-lg hover:bg-green-700">{{ __("Accept") }}</button>
                            <button name="decision" value="reject" class="bg-red-600 text-white px-4 py-2 rounded-lg hover:bg-red-700">{{ __("Reject") }}</button>
                        </form>
                    </div>
                @empty
                    <p class="text-slate-600 dark:text-slate-300">{{ __("No incoming movement requests.") }}</p>
                @endforelse
            </div>
        </section>

        <section class="uniform-card p-6">
            <h2 class="text-xl font-semibold text-slate-900 dark:text-slate-100 mb-4">{{ __("Movement Requests Awaiting Approval") }}</h2>
            <div class="space-y-3">
                ...
                @empty
                    <p class="text-slate-600 dark:text-slate-300">{{ __("No pending movement requests.") }}</p>
                @endforelse
            </div>
        </section>

        <section class="uniform-card p-6">
            <h2 class="text-xl font-semibold text-slate-900 dark:text-slate-100 mb-4">{{ __("Available Assets") }}</h2>
            <div class="space-y-5">
                @forelse($assets as $asset)
                    <div class="rounded-lg border border-slate-200 dark:border-slate-700 p-4">
                        ...
                        <form method="POST" action="{{ route('manager.assets.movements.request', $asset) }}" class="mt-4 grid grid-cols-1 md:grid-cols-3 gap-3">
                            ...
                            <input type="text" name="request_note" placeholder="{{ __("Movement note") }}" class="md:col-span-2 px-3 py-2 rounded-lg border border-slate-300 dark:border-slate-700 bg-white dark:bg-slate-900 text-slate-900 dark:text-slate-100">
                            <button type="submit" class="md:col-span-3 bg-amber-600 text-white px-4 py-2 rounded-lg hover:bg-amber-700">{{ __("Request Movement") }}</button>
                        </form>

                        <form method="POST" action="{{ route('manager.assets.issues.store', $asset) }}" class="mt-4 grid grid-cols-1 md:grid-cols-3 gap-3">
                            @csrf
                            <input type="text" name="title" placeholder="{{ __("Issue title") }}" class="px-3 py-2 rounded-lg border border-slate-300 dark:border-slate-700 bg-white dark:bg-slate-900 text-slate-900 dark:text-slate-100" required>
                            ...
                            <button type="submit" class="bg-blue-600 text-white px-4 py-2 rounded-lg hover:bg-blue-700">{{ __("Report Issue") }}</button>
                            <textarea name="description" placeholder="{{ __("Describe the issue") }}" class="md:col-span-3 px-3 py-2 rounded-lg border border-slate-300 dark:border-slate-700 bg-white dark:bg-slate-900 text-slate-900 dark:text-slate-100" rows="3" required></textarea>
                        </form>
                    </div>
                @empty
                    <p class="text-slate-600 dark:text-slate-300">{{ __("No assets assigned to your managed hostels.") }}</p>
                @endforelse
            </div>
        </section>

        <section class="uniform-card p-6">
            <h2 class="text-xl font-semibold text-slate-900 dark:text-slate-100 mb-4">{{ __("Recent Asset Issue Reports") }}</h2>
            <div class="space-y-3">
                ...
                @empty
                    <p class="text-slate-600 dark:text-slate-300">{{ __("No issue reports yet.") }}</p>
                @endforelse
            </div>
        </section>
    </div>
