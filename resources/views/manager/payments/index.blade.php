<x-dashboard-layout title="Payments">
    <div class="max-w-6xl mx-auto">
        <div class="mb-8">
            ...
            <p class="text-gray-600 mt-2">View all payments from students in your hostel</p>
        </div>
                        ...
                        <tbody class="divide-y divide-gray-200">
                            @foreach($payments as $payment)
                                <tr class="hover:bg-gray-50">
                                    <td class="px-6 py-4 text-sm text-gray-900">
                                        <div class="flex items-center gap-3">
                                            @if($payment->user->profile_image)
                                                <img src="{{ asset('storage/' . $payment->user->profile_image) }}" alt="{{ $payment->user->name }}" class="w-8 h-8 rounded-full object-cover">
                                            @else
                                                <div class="w-8 h-8 rounded-full bg-gray-300 flex items-center justify-center">
                                                    <span class="text-xs font-semibold text-gray-700">{{ substr($payment->user->name, 0, 1) }}</span>
                                                </div>
                                            @endif
                                            <div>
                                    ...
                                    <td class="px-6 py-4 text-sm text-gray-900">
                                        <a href="{{ route('manager.bookings.show', $payment->booking_id) }}" class="text-blue-600 hover:underline">
                                            #{{ $payment->booking_id }}
                                        </a>
                                    </td>
        ...
        @else
            <div class="bg-white rounded-lg shadow p-8 text-center">
                ...
                <p class="text-gray-600">No payments have been recorded for your hostel.</p>
            </div>
        @endif
    </div>
</x-dashboard-layout>
