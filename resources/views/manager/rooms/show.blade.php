    ...
    <div class="max-w-5xl mx-auto space-y-6">
        <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
            ...
            <div class="flex items-center gap-2">
                ...
                <form method="POST" action="{{ route('manager.rooms.destroy', $room) }}" onsubmit="return confirm(@js(__('Are you sure?')))" class="inline">
                    @csrf
                    @method('DELETE')
                    <button type="submit" class="bg-red-600 text-white px-4 py-2.5 rounded-lg hover:bg-red-700">{{ __("Delete") }}</button>
                </form>
            </div>
        </div>
        ...
        @if($room->description)
            <section class="bg-white dark:bg-slate-900 rounded-lg shadow-md border border-transparent dark:border-slate-800 p-6">
                <h2 class="text-lg font-semibold text-slate-900 dark:text-slate-100 mb-3">{{ __("Description") }}</h2>
        ...
        <section class="bg-white dark:bg-slate-900 rounded-lg shadow-md border border-transparent dark:border-slate-800 p-6">
            <div class="flex items-center justify-between mb-4">
                <h2 class="text-xl font-semibold text-slate-900 dark:text-slate-100">{{ __("Beds") }}</h2>
                <span class="text-xs text-slate-500 dark:text-slate-400">{{ __('Only admin-approved beds are visible to students.') }}</span>
            </div>
            ...
            <form method="POST" action="{{ route('manager.rooms.beds.store', $room) }}" class="grid grid-cols-1 md:grid-cols-4 gap-3 mb-6 p-4 bg-slate-50 dark:bg-slate-800 rounded-lg border border-slate-200 dark:border-slate-700">
                @csrf
                <div>
                    <label class="block text-sm text-slate-700 dark:text-slate-300 mb-1">{{ __("Bed Number") }}</label>
                ...
                <div class="flex items-end">
                    <button type="submit" class="w-full bg-blue-600 text-white py-2 rounded-lg hover:bg-blue-700">{{ __("Add Bed Space") }}</button>
                </div>
            </form>
