...
<head>
    ...
    @php
        $customCssEnabled = filter_var(\App\Models\SystemSetting::getSetting('custom_css_enabled', true), FILTER_VALIDATE_BOOL);
    ...
    @endphp
    @if($customCssEnabled && !empty($customCss))
...
<body class="h-full bg-gray-50 dark:bg-slate-950 text-gray-900 dark:text-slate-100 antialiased">
    @php
        ...
        if ($currentUser && ($currentUser->isStudent() || $currentUser->isManager())) {
            ...
            $query = \App\Models\PopupAnnouncement::query()
                ->currentlyActive()
                ->whereIn('target', [
                    $roleTarget,
                    $currentUser->isStudent() ? 'student' : 'manager',
                    'both',
                    'all',
                ]);

            $seenPopupIds = collect(session('seen_popup_ids', []))
                ->map(fn ($id) => (int) $id)
                ->filter()
                ->values()
                ->all();
            ...
            $loginPopups = $query
                ->latest('id')
                ->get()
                ->reject(fn ($popup) => in_array((int) $popup->id, $seenPopupIds, true))
                ->values();

            if ($loginPopups->isNotEmpty()) {
                $allSeen = array_values(array_unique(array_merge(
                    $seenPopupIds,
                    $loginPopups->pluck('id')->map(fn ($id) => (int) $id)->all()
                )));
                session(['seen_popup_ids' => $allSeen]);
            }
        }
    ...
    @if($loginPopups->isNotEmpty())
        <div id="login-popup-modal" class="fixed inset-0 z-[90] flex items-center justify-center p-4" style="position:fixed;inset:0;z-index:2147483647;display:flex;visibility:visible;opacity:1;">
            <div class="absolute inset-0 bg-black/60" aria-hidden="true"></div>
            <div class="relative w-full max-w-3xl rounded-xl bg-white dark:bg-slate-900 border border-gray-200 dark:border-slate-700 shadow-2xl">
                <div class="px-5 py-4 border-b border-gray-200 dark:border-slate-700 flex items-center justify-between">
                    <h3 class="text-base font-semibold text-gray-900 dark:text-slate-100">{{ __('Announcements') }}</h3>
                    <button type="button" id="login-popup-close-all" class="text-sm text-gray-500 hover:text-gray-700 dark:text-slate-400 dark:hover:text-slate-200">{{ __('Close all') }}</button>
                </div>
                <div id="login-popup-list" class="px-5 py-4 space-y-4 max-h-[70vh] overflow-y-auto">
                    @foreach($loginPopups as $popup)
                        <article data-popup-id="{{ $popup->id }}" class="rounded-lg border border-gray-200 dark:border-slate-700">
                            <div class="px-4 py-3 border-b border-gray-200 dark:border-slate-700 flex items-center justify-between">
                                <h4 class="text-sm font-semibold text-gray-900 dark:text-slate-100">{{ $popup->title }}</h4>
                                <button
                                    type="button"
                                    class="login-popup-dismiss text-xs bg-blue-600 text-white px-3 py-1.5 rounded-md hover:bg-blue-700"
                                    data-dismiss-url="{{ route('notifications.popup.dismiss', $popup) }}"
                                    data-popup-id="{{ $popup->id }}"
                                >
                                    {{ __('Close') }}
                                </button>
                            </div>
                            <div class="px-4 py-3 text-sm text-gray-700 dark:text-slate-200 prose prose-sm max-w-none dark:prose-invert">
                                {!! $popup->body !!}
                            </div>
                        </article>
                    @endforeach
    ...
    <script>
        (function () {
            ...
            const popupModal = document.getElementById('login-popup-modal');
            if (popupModal) {
                popupModal.style.setProperty('display', 'flex', 'important');
                popupModal.style.setProperty('visibility', 'visible', 'important');
                popupModal.style.setProperty('opacity', '1', 'important');
                popupModal.style.setProperty('position', 'fixed', 'important');
                popupModal.style.setProperty('inset', '0', 'important');
                popupModal.style.setProperty('z-index', '2147483647', 'important');
                popupModal.style.setProperty('pointer-events', 'auto', 'important');
                popupModal.style.setProperty('align-items', 'center', 'important');
                popupModal.style.setProperty('justify-content', 'center', 'important');
            }

            const popupDismissButtons = Array.from(document.querySelectorAll('.login-popup-dismiss'));
            const closeAllBtn = document.getElementById('login-popup-close-all');

            popupDismissButtons.forEach((btn) => {
                btn.addEventListener('click', async function () {
                    const modal = document.getElementById('login-popup-modal');
                    const article = btn.closest('article[data-popup-id]');
                    const list = document.getElementById('login-popup-list');
                    ...
                    try {
                        ...
                        // Ignore dismiss network failure.
                    }

                    article?.remove();
                    if (!list || list.querySelectorAll('article[data-popup-id]').length === 0) {
                        modal?.remove();
                    }
                });
            });
