<x-dashboard-layout :title="__('Student Dashboard')">
    <x-slot name="sidebar">
    ...
    <div class="uniform-page">
        <section class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
            <div>
                <h1 class="text-3xl font-bold text-gray-900 dark:text-slate-100">{{ __('Student Dashboard') }}</h1>
                <p class="text-gray-600 dark:text-slate-300 mt-1">{{ __('Track your bookings, payments, and complaints in one place.') }}</p>
            </div>
        ...
        <section class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-5 gap-6">
            <div class="uniform-card p-6">
                <p class="text-gray-600 dark:text-slate-300 text-sm">{{ __('Active Stay') }}</p>
                <p class="text-3xl font-bold {{ $stats['active_booking'] ? 'text-green-600 dark:text-green-400' : 'text-gray-900 dark:text-slate-100' }}">{{ $stats['active_booking'] ? __('Yes') : __('No') }}</p>
            </div>
            <div class="uniform-card p-6">
            ...
            <div class="uniform-card p-6">
                <p class="text-gray-600 dark:text-slate-300 text-sm">{{ __('Completed Bookings') }}</p>
                <p class="text-3xl font-bold text-blue-600 dark:text-blue-400">{{ $stats['completed_bookings'] }}</p>
            </div>
            <div class="uniform-card p-6">
                <p class="text-gray-600 dark:text-slate-300 text-sm">{{ __('Total Paid') }}</p>
        ...
        <section class="grid grid-cols-1 lg:grid-cols-2 gap-8">
            <div class="uniform-card p-6">
                <h2 class="text-2xl font-bold text-gray-900 dark:text-slate-100 mb-4">{{ __("Current Stay") }}</h2>
                @if($currentBooking)
                    ...
                    <div class="mt-4 pt-4 border-t border-gray-200 dark:border-slate-700 grid grid-cols-1 sm:grid-cols-2 gap-4">
                        <div>
                            <p class="text-sm text-gray-600 dark:text-slate-300">{{ __("Outstanding Balance") }}</p>
                        ...
                        <div>
                            <p class="text-sm text-gray-600 dark:text-slate-300">{{ __('Days to Checkout') }}</p>
                            <p class="text-xl font-bold text-gray-900 dark:text-slate-100">{{ $stats['days_to_checkout'] ?? 'N/A' }}</p>
                        </div>
                    </div>
                @else
                    <p class="text-gray-600 dark:text-slate-300 mb-4">{{ __("No active stay yet.") }}</p>
                    <a href="{{ route('student.bookings.available') }}" class="inline-flex items-center bg-blue-600 text-white px-4 py-2 rounded-lg hover:bg-blue-700 font-medium">{{ __("Browse Rooms") }}</a>
                @endif
            </div>

            <div class="uniform-card p-6">
                <h2 class="text-2xl font-bold text-gray-900 dark:text-slate-100 mb-4">{{ __("Quick Actions") }}</h2>
                <div class="space-y-3">
                    <a href="{{ route('student.bookings.available') }}" class="block w-full text-center bg-blue-600 text-white py-2 rounded-lg hover:bg-blue-700 font-medium">{{ __("Browse Rooms") }}</a>
                    <a href="{{ route('student.bookings.index') }}" class="block w-full text-center bg-gray-100 dark:bg-slate-800 text-gray-800 dark:text-slate-100 py-2 rounded-lg hover:bg-gray-200 dark:hover:bg-slate-700 font-medium">{{ __("My Bookings") }}</a>
                    <a href="{{ route('student.payments.index') }}" class="block w-full text-center bg-gray-100 dark:bg-slate-800 text-gray-800 dark:text-slate-100 py-2 rounded-lg hover:bg-gray-200 dark:hover:bg-slate-700 font-medium">{{ __("Payment History") }}</a>
                    <a href="{{ route('student.room-change.index') }}" class="block w-full text-center bg-gray-100 dark:bg-slate-800 text-gray-800 dark:text-slate-100 py-2 rounded-lg hover:bg-gray-200 dark:hover:bg-slate-700 font-medium">{{ __("Change Room") }}</a>
                    <a href="{{ route('student.hostel-change.index') }}" class="block w-full text-center bg-gray-100 dark:bg-slate-800 text-gray-800 dark:text-slate-100 py-2 rounded-lg hover:bg-gray-200 dark:hover:bg-slate-700 font-medium">{{ __("Change Hostel") }}</a>
                    <a href="{{ route('student.complaints.index') }}" class="block w-full text-center bg-gray-100 dark:bg-slate-800 text-gray-800 dark:text-slate-100 py-2 rounded-lg hover:bg-gray-200 dark:hover:bg-slate-700 font-medium">{{ __("Support & Complaints") }}</a>
                </div>
            </div>
        </section>
        ...
        <section class="grid grid-cols-1 lg:grid-cols-2 gap-8">
            <div class="uniform-card p-6">
                ...
                <div class="space-y-3">
                    ...
                    @empty
                        <p class="text-gray-600 dark:text-slate-300">{{ __("No booking history yet.") }}</p>
                    @endforelse
                </div>
            </div>
        ...
        <section class="uniform-card p-6">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-4">
                ...
                @empty
                    <p class="text-gray-600 dark:text-slate-300">{{ __("No complaints submitted.") }}</p>
                @endforelse
            </div>
        </section>
    </div>
