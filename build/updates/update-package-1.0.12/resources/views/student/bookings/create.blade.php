                    ...
                    <div class="bg-blue-50 dark:bg-blue-900/30 border border-blue-200 dark:border-blue-800 rounded-lg p-4">
                        <div class="text-sm text-blue-800 dark:text-blue-200">
                            ...
                            @if($periodType === 'semesters' && ($trimesterBookingEnabled ?? false))
                                <p class="mt-1 text-xs text-blue-700 dark:text-blue-300">
                                    Trimester payable: {{ getCurrencySymbol() }}{{ number_format($trimesterPrice ?? 0, 2) }} (session base ÷ 3, session discount not applied)
                                </p>
                            @endif
                            @if(($bookingDiscountPreview['amount'] ?? 0) > 0)
                                <p class="mt-1 text-xs text-blue-700 dark:text-blue-300">
                                    Eligible discount: {{ $bookingDiscountPreview['rule']['name'] ?? 'Booking Discount' }}
                                    ({{ getCurrencySymbol() }}{{ number_format((float) ($bookingDiscountPreview['amount'] ?? 0), 2) }} off).
                                    Final discount is applied on submit based on selected booking type and active rules.
                                </p>
                            @else
                                <p class="mt-1 text-xs text-blue-700 dark:text-blue-300">
                                    Additional admin-configured booking discounts (if eligible) are applied on submit.
                                </p>
                            @endif
                        </div>
                    </div>
