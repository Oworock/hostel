...
@php
    ...
    $trimesterEligibleSchools = json_decode(\App\Models\SystemSetting::getSetting('trimester_eligible_schools_json', ''), true);
    ...
    if ($registrationSchoolOptions->isEmpty()) {
        $registrationSchoolOptions = collect(is_array($trimesterEligibleSchools) ? $trimesterEligibleSchools : [])->map(fn ($school) => trim((string) $school))->filter()->unique()->values();
    }
